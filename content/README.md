...
- `venue.js`: settings for the room. Where the real statue stands in the projection (a black mask so
  nothing is projected onto her), whether the statue photo is shown, which part of the camera image is
...
black shape covers the statue with a little margin. → *Kamera*: frame the area where visitors' hands move.
